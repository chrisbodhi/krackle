...
export interface KrackleOptions {
  maxParticles?: number;        // pool size (default 920)
  lifespan?: [number, number];  // ms, min/max (default [1200, 2500])
  zIndex?: number;              // canvas z-index (default 10)
  seed?: number;                // sprite atlas seed (default 19620828: Kirby's birthday, Aug 28, on 1962 — the Marvel Age year he co-created Thor & the Hulk)
  ...
  lobesMax?: number;            // silhouette lobes 1-5 (default 5)
  lobeSpread?: number;          // 0.2-0.9 (default 0.55)
  rimWidth?: number;            // px (default 1.5)
  satsMax?: number;             // satellites per cluster max (default 2)
  satSpread?: number;           // ×anchor radius (default 1.55)
  rays?: [number, number];      // white rays per burst, min/max (default [2, 4])
  rayHalf?: [number, number];   // ray half-angle range, radians (default [0.32, 0.44])
  burstRadius?: [number, number];// inner halo / outer rim, px (default [22, 210])
  anchorRadius?: [number, number];// cluster-core dot radius, px (default [6, 14.5])
  satRadius?: [number, number]; // satellite dot radius, px (default [4.5, 10])
  dissolve?: number;            // 0-1, how much satellites shrink toward the rim (default 0.15)
  driftRange?: [number, number];// base outward drift per cluster, px (default [34, 106])
  bandAttempts?: [number, number, number]; // cluster attempts per band, inner→outer (default [14, 18, 26])
  bandDelayMs?: number;         // ms between band bloom stagger (default 85)
  flash?: "none" | "white" | "invert"; // full-viewport flash on click (default "invert")
